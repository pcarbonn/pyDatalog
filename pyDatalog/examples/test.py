...
def test():
    ...
    def equality():
        ...
        # modulo
        assert ask((X==5) & (Y==2) & (Z==X%Y)) == set([(5,2,1)])
        assert ask((X==5) & (Z==X%2)) == set([(5,1)])
        assert ask((X==5) & (Z==6%X)) == set([(5,1)])
    ...
    # issue #31 (redefining a predicate to empty)
    @pyDatalog.program()
    def issue31():
        +A31(2)
        +A31(3)
        B31(X31) <= A31(X31)
        +B31(4)
        assert ask(B31(X31)) == set([(2,), (3,), (4,)])

        # redefine B31 to be empty
        B31(X31) <= None
        assert ask(B31(X31)) == None # no results found (not throwing AttributeError)
        assert B31(X31) == []

        # testing del f31[X31] and del f31[constant]
        f31[1] = 2
        f31[2] = 3
        (f31[X31] == Y31) <= (X31.in_((3, 4))) & (Y31 == X31 + 10)
        assert ask(f31[X31] == Y31) == set([(1, 2), (2, 3), (3, 13), (4, 14)])

        # del f31[X31] should only delete rules, keeping facts
        del f31[X31]
        assert ask(f31[X31] == Y31) == set([(1, 2), (2, 3)])

        # del f31[constant] should delete the specific fact
        del f31[1]
        assert ask(f31[X31] == Y31) == set([(2, 3)])
